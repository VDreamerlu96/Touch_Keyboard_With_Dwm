...
static void Change_key_or_full(const Arg* arg);
...
void tile_with_keyboard(Monitor *m) {
    unsigned int i, n, h, mw, my, ty;
    Client *c;
    ...
    // 计算左侧和右侧的宽度
    int left_width = m->ww / Width_Cut_Num * Left_Board_Num;  // 左侧占1/10
    int right_width = m->ww / Width_Cut_Num * Right_Board_Num;  // 右侧占2/10
    int available_width = m->ww - left_width - right_width;  // 剩余区域7/10

    // 计算主区域的宽度（主区域的宽度根据窗口数量和 m->nmaster 来调整）
    if (n > m->nmaster)
        mw = m->nmaster ? available_width * m->mfact : 0;
    else
        mw = available_width;
    ...
    for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < m->nmaster) {
            // 对主区域窗口进行布局
            h = (m->wh - my) / (MIN(n, m->nmaster) - i);
            resize(c, m->wx + left_width, m->wy + my, mw - (2 * c->bw), h - (2 * c->bw), 0);
            if (my + HEIGHT(c) < m->wh)
                my += HEIGHT(c);
        } else {
            // 对从区域窗口进行布局
            h = (m->wh - ty) / (n - i);
            resize(c, m->wx + left_width + mw, m->wy + ty, available_width - mw - (2 * c->bw), h - (2 * c->bw), 0);
            if (ty + HEIGHT(c) < m->wh)
                ty += HEIGHT(c);
        }
    }
...
void tile(Monitor* m){
	if(key_or_full){
		...
		tile_with_keyboard(m);
	}
}

static void Change_key_or_full(){
	if(key_or_full){
		key_or_full = 0;
	}else{
		key_or_full = 1;
