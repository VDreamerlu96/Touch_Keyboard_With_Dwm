...
void tile_with_keyboard(Monitor *m) { 
    unsigned int i, n, h, mw, my, ty, total_height;
    Client *c;
    ...
    // 计算可用的屏幕宽度（无左右留白）
    int available_width = m->ww;

    // 计算下方留白
    int down_margin = m->wh / Height_Cut_Num * Down_Board_Num;  // 下方留白
    ...
    // 计算布局的可用总高度（去除 `Board_Start_Height`）
    total_height = m->wh - down_margin;

    // 布局窗口
    for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < m->nmaster) {
            // 主区域窗口布局
            h = (total_height - my) / (MIN(n, m->nmaster) - i);
            resize(c, m->wx, m->wy + my, mw - (2 * c->bw), h - (2 * c->bw), 0);
            if (my + HEIGHT(c) < total_height)
                my += HEIGHT(c);
        } else {
            // 副区域窗口布局
            h = (total_height - ty) / (n - i);
            resize(c, m->wx + mw, m->wy + ty, available_width - mw - (2 * c->bw), h - (2 * c->bw), 0);
            if (ty + HEIGHT(c) < total_height)
                ty += HEIGHT(c);
        }
    }
}
